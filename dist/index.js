...
if (args.length > 0) {
    file = path_1.default.resolve(args[0]);
    screen.title = `Notes App - ${path_1.default.basename(file)}`;
...
let timeout = null;
function status(message, duration = 2000) {
    if (timeout) {
...
const editor = blessed_1.default.box({
    parent: screen,
    top: 0,
    left: 0,
    width: '100%',
...
const popup = blessed_1.default.list({
    parent: screen,
    left: 0,
    top: 0,
    width: 20,
...
function display(line, current, index) {
    if (current) {
        return line;
    }
    ...
    if (block.inBlock) {
        try {
            const highlighted = (0, cli_highlight_1.highlight)(line, { language: block.lang || 'plaintext', ignoreIllegals: true });
    ...
    if (line.startsWith('# ')) {
        ...
        try {
            const ascii = figlet_1.default.textSync(text, {
                font: 'Standard',
    ...
    if (key.full === 'C-s') {
        if (file) {
            try {
                ...
                status(` Saved: ${path_1.default.basename(file)}`);
            }
            catch (err) {
